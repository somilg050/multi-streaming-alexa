...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (!playbackInfo.hasPreviousPlaybackSession) {
            message = 'Welcome to the Music Ground. You can say, play the music, to begin.';
            reprompt = 'You can say, play the music, to begin.';
            await shuffleOrder();
        } else {
            playbackInfo.inPlaybackSession = false;
            message = `You were listening to ${constants.audioData[playbackInfo.playOrder[playbackInfo.index]][1]}. Would you like to resume?`;
...
const HelpHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (!playbackInfo.hasPreviousPlaybackSession) {
            message = 'Welcome to the Music Ground. you can ask to play the music to begin the stream.';
        } else if (!playbackInfo.inPlaybackSession) {
            message = `You were listening to ${constants.audioData[playbackInfo.index][1]}. Would you like to resume?`;
        } else {
            message = 'Welcome to the Music Ground. You can say, Next or Previous to navigate through the playlist. At any time, you can say Pause to pause the audio and Resume to resume.';
        }
...
const controller = {
    async play(handlerInput) {
        ...
        if(index === 0) {
            ...
            index = randomIndex;
            track = constants.audioData[playOrder[randomIndex]];
        } else {
            track = constants.audioData[playOrder[index]];
        }
        ...
        let audioItemMetadata = {
            "title": track[1],
            "subtitle": "MUSICGROUND",
            "art": {
                "sources": [
                  {
                    "contentDescription": "MUSICGROUND",
                    "url": "https://inside-the-box-images.s3.amazonaws.com/Music+Ground.jpeg",
                    "widthPixels": 512,
                    "heightPixels": 512
                  }
                ]
        ...
        if (handlerInput.requestEnvelope.request.type.startsWith('PlaybackController.')) {
            responseBuilder
                .withShouldEndSession(true)
                .addAudioPlayerPlayDirective(playBehavior, track[0], token, offsetInMilliseconds, null, audioItemMetadata);
            return responseBuilder.getResponse();
        }
        
        let speech = "";
        if (playbackInfo.hasPreviousPlaybackSession) {
            speech = `This is ${track[1]}.`;
        } else {
            speech = `This is ${track[1]}.`
        }
        responseBuilder
